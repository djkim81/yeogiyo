...
$(document).ready(function() {
  ...
  if (typeof redirectUrl !== "undefined" && redirectUrl !== null && redirectUrl.length > 0) {
    $(".util-list").children("li").eq(0).children("a").click(function() {
      
      window.open(uri + "/cmn/cmn/password_reset_request?redirect_url=" + encodeURIComponent(redirectUrl), "_suta_password_reset");
    });
    $(".util-list").children("li").eq(1).children("a").click(function() {
      window.open(uri + "/cmn/cmn/register?redirect_url=" + encodeURIComponent(redirectUrl), "_suta_register");
    });
  } else {
    $(".util-list").children("li").eq(0).children("a").click(function() {
      window.open(uri + "/cmn/cmn/password_reset_request", "_suta_password_reset");
    });
    $(".util-list").children("li").eq(1).children("a").click(function() {
      window.open(uri + "/cmn/cmn/register", "_suta_register");
    });
  }
  var loginChkYn = getParameter("login_chk_yn");
  if (loginChkYn === "Y") {
    cmnSyncCall("LoginChk", {}, callback, null);
  }
});
...
function callback(data, act, input_param, callbackVar) {
  if (act == "TempLogin") {
    ...
    window.parent.postMessage({messageTyp: "login_fnsh"}, "*");
    $("#_cmn_loader").show();
  ...
  } else if (act === "LoginChk") {
    if (data.login_chk_yn === true) {
      window.parent.postMessage({messageTyp: "login_fnsh"}, "*");
      $("#_cmn_loader").show();
